...
def IS_estimate(x, enc, dec, ll, mask, K = 100, sample_W = True, mu_pz = 0.0, log_sig_pz = 0.0):
    if len(x.get_shape().as_list()) == 4:
        x_rep = tf.tile(x, [K, 1, 1, 1])
    if len(x.get_shape().as_list()) == 2:
        x_rep = tf.tile(x, [K, 1]) # 扩展x，这里把x按行方向复制了100次，x_rep.shape=(5000, 784)
    N = x.get_shape().as_list()[0]
    mu_qz, log_sig_qz = enc(x_rep)  # 把x_rep放进encoder，得到输出
    z = sample_gaussian(mu_qz, log_sig_qz)
    mu_x = dec(z, mask, sampling = sample_W)  # 把z传进decoder，得到输出
...
def construct_eval_func(X_ph, enc, dec, ll, batch_size_ph, mask, K = 100, sample_W = True):
    test_ll_mean, test_ll_var = IS_estimate(X_ph, enc, dec, ll, mask, K, sample_W)
