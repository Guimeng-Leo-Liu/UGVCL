...
def KL_generated_images(dec, cla, N, dimZ, task, mask, sample_W = True):
    z = tf.random_normal(shape=(N, dimZ))
    x_gen = dec(z, mask, sampling = sample_W)
...
def construct_eval_func(dec, cla, batch_size_ph, dimZ, task, mask, sample_W = True):
    N_gen = 100
    kl_mean, kl_var = KL_generated_images(dec, cla, N_gen, dimZ, task, mask, sample_W)
    ops = [kl_mean, kl_var]
